...
class Photo : AppCompatActivity() {
    ...
    private fun saveFavoriteBanner(imagePath: String) {
        ...
        prefs.edit().putString(KEY_FAVORITE_BANNER, imagePath).apply()
    }

    private fun addToFavorites(imagePath: String) {
        val prefs = getSharedPreferences(PREFS_NAME, MODE_PRIVATE)
        val currentFavorites = prefs.getStringSet(KEY_FAVORITE_IMAGES, emptySet())?.toMutableSet()
            ?: mutableSetOf()
        currentFavorites.add(imagePath)
        prefs.edit().putStringSet(KEY_FAVORITE_IMAGES, currentFavorites).apply()
    }
